    ...
    case "JOIN":
      if (stateChange.members.size === 1 && player.paused) {
        let emb = new MessageEmbed()
          .setAuthor(`Reanudación de la cola en pausa`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(
            `Reanudando la reproducción porque todos ustedes me dejaron con música para tocar sola`
          );
    ...
    case "LEAVE":
      if (stateChange.members.size === 0 && !player.paused && player.playing) {
        ...
        let emb = new MessageEmbed()
          ...
          .setColor(client.botconfig.EmbedColor)
          .setDescription(`El reproductor ha sido pausado porque todos se fueron`);
        await client.channels.cache.get(player.textChannel).send(emb);
      }
      break;
  }

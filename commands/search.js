...
module.exports = {
  name: "search",
  description: "Muestra un resultado de canciones basado en la consulta de búsqueda",
  usage: "[canción]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Uso - **\`${GuildDB.prefix}search [canción]\``
      );
    let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
    if (!CheckNode || !CheckNode.connected) {
      return client.sendTime(
        message.channel,
        "❌ | **No conectada a lavalink**"
      );
    }
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Tardaste demasiado en responder. ¡Ejecuta el comando de nuevo si quieres reproducir algo!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Envía el número de identificación de la canción correcto"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "No se encontró ninguna canción para la identificación dada"
        );
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "TRACK_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Añadido a la cola:** \`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}}\`.`
            );
        ...
        switch (res.loadType) {
          ...
          case "SEARCH_RESULT":
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `El número que proporcionó fue mayor o menor que el total de la búsqueda. Uso - \`(1-${max})\``
              );

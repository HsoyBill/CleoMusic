...
module.exports = {
  name: "config",
  description: "Editar la configuración del bot",
  usage: "",
  permissions: {
...
`);
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(
        message.channel,
        "¿A qué quieres cambiar el prefix?"
      );
      ...
      client.sendTime(
        message.channel,
        `Prefix del servidor guardado con éxito como \`${prefix}\``
      );
    } else {
      await client.sendTime(
        message.channel,
  ...
  SlashCommand: {
    options: [
      {
        name: "prefix",
        description: "Verifique el prefix del bot",
        type: 1,
        required: false,
        options: [
          {
            name: "symbol",
            description: "Establecer el prefix del bot",
            type: 3,
            required: false,
          },
        ],
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `El prefijo de este server \`${GuildDB.prefix}\``
          );
        }
